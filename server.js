...
async function boot() {
  ...
  await sendEmail(
    ...
    [
      `Marshall Command Layer v${MARSHALL_VERSION} is now active in SHADOW-ONLY mode.`,
      `Webhook signature verification: ${CONFIG.RESEND_WEBHOOK_SECRET ? "ENFORCED" : "BLOCKED"}`,
      "",
      "Order submission: DISABLED in source",
      "Position closing: DISABLED in source",
      "This service cannot place, close, liquidate, or rebalance trades.",
      "",
      "Available read-only command:",
      "  APEX STATUS         — paper-account portfolio snapshot",
      "",
      "Any legacy trade-command email is recorded and blocked; it cannot override this safety lock.",
      "",
      `${etNow().toLocaleString()} ET`,
    ].join("\n")
  );
